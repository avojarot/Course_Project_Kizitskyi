    ...
    public partial class HomeForm : Form
    {
       ...
       protected void exitToolStripMenuItem_Click(object sender, EventArgs e)
        ...
        private void ArtSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                articlesBindingSource.DataSource = search.Searched;
                if (search.Searched.Count == 0)
                    MessageBox.Show("К сожалению, ничего не найден...", "Ошибка");
            }
            else if (search.DialogResult == DialogResult.No)
                articlesBindingSource.DataSource = guide.Articles;
        ...
        {
            this.Hide();
            var testf = new TESThomeForm(guide);
           testf.ShowDialog();
